...
public:
    ComMtaScope() {
        ...
        else if (hr != RPC_E_CHANGED_MODE) internal::ThrowIfFailed(hr, L"CoInitializeEx(video capture thread)");
    }
...
std::size_t DefaultClonePoolSize(const MFD3D12VideoCaptureThreadConfig& config) {
...
struct MFD3D12VideoCaptureThread::Impl {
    ...
    std::vector<MFD3D12VideoFrameQueuePtr> liveQueuesSnapshot() {
        std::vector<MFD3D12VideoFrameQueuePtr> out;
    ...
    void waitForPlaybackTime(MFD3D12VideoFrame& frame,
        ...
        while (!token.stopRequested()) {
            ...
            for (auto& q : queuesSnapshot) {
                ...
                try {
                    ...
                    setLastError(internal::MakeError(L"MFD3D12VideoCaptureThread::clone/push", internal::Utf8ToWide(e.what())));
                }
            }
        }
    }
...
MFD3D12VideoCaptureThread::MFD3D12VideoCaptureThread()
...
bool MFD3D12VideoCaptureThread::open(const MFD3D12VideoCaptureThreadConfig& config,
                                     std::shared_ptr<D3D12CoreLib::D3D12Core> d3d12) {
    close();
    try {
        if (!d3d12) throw std::runtime_error("MFD3D12VideoCaptureThread::open: null D3D12Core");
        if (!impl_->capture.open(config.filePath, config.capture, d3d12)) {
        ...
        impl_->setLastError(internal::MakeError(L"MFD3D12VideoCaptureThread::open", internal::Utf8ToWide(e.what())));
        close();
        return false;
    }
}

void MFD3D12VideoCaptureThread::start() {
    if (!impl_ || !impl_->opened) throw std::logic_error("MFD3D12VideoCaptureThread::start: not opened");
    if (impl_->worker.joinable()) throw std::logic_error("MFD3D12VideoCaptureThread::start: already started");
    impl_->worker.start([this](const ThreadKit::StopToken& token) { impl_->workerLoop(token); });
}

void MFD3D12VideoCaptureThread::requestStop() noexcept {
    if (impl_) impl_->worker.requestStop();
}

void MFD3D12VideoCaptureThread::stop() {
    if (impl_) impl_->worker.stopAndJoin();
}

void MFD3D12VideoCaptureThread::close() noexcept {
    if (!impl_) return;
...
bool MFD3D12VideoCaptureThread::isOpened() const noexcept { return impl_ && impl_->opened; }
bool MFD3D12VideoCaptureThread::isRunning() const noexcept { return impl_ && impl_->worker.isRunning(); }

MFD3D12VideoFrameQueuePtr MFD3D12VideoCaptureThread::createQueue() {
    if (!impl_) throw std::logic_error("MFD3D12VideoCaptureThread::createQueue: invalid object");
...
MFD3D12VideoFrameQueuePtr MFD3D12VideoCaptureThread::createQueue(
    ...
    auto q = std::make_shared<MFD3D12VideoFrameQueue>(MakeQueueOptions(capacity, overflowPolicy));
    registerQueue(q);
    return q;
}

void MFD3D12VideoCaptureThread::registerQueue(const MFD3D12VideoFrameQueuePtr& queue) {
    if (!queue) throw std::invalid_argument("MFD3D12VideoCaptureThread::registerQueue: null queue");
    std::lock_guard<std::mutex> lock(impl_->mutex);
    impl_->queues.emplace_back(queue);
}

void MFD3D12VideoCaptureThread::unregisterExpiredAndClosedQueues() {
    if (impl_) (void)impl_->liveQueuesSnapshot();
}

MFD3D12VideoCaptureThreadStats MFD3D12VideoCaptureThread::stats() const noexcept {
    MFD3D12VideoCaptureThreadStats s;
    if (!impl_) return s;
...
void MFD3D12VideoCaptureThread::resetStats() noexcept {
    if (!impl_) return;
...
const MFErrorInfo& MFD3D12VideoCaptureThread::lastError() const noexcept { return impl_->lastError; }
const MFCameraFormatInfo& MFD3D12VideoCaptureThread::selectedFormat() const noexcept { return impl_->capture.selectedFormat(); }
std::exception_ptr MFD3D12VideoCaptureThread::workerException() const { return impl_ ? impl_->worker.exception() : nullptr; }
void MFD3D12VideoCaptureThread::rethrowWorkerExceptionIfAny() const { if (impl_) impl_->worker.rethrowIfException(); }
