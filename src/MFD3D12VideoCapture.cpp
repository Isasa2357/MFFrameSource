...
MFD3D12VideoCapture::MFD3D12VideoCapture()
...
bool MFD3D12VideoCapture::open(const std::wstring& filePath,
    ...
    try {
        if (!d3d12) {
            throw std::runtime_error("MFD3D12VideoCapture::open: null D3D12Core");
        }
        if (!config.input.isComplete()) {
            throw std::runtime_error("MFD3D12VideoCapture::open: config.input must be exact decoded format request");
        }
        ...
        if (!IsSupportedCpuUploadInputFormat(requestedDxgi)) {
            throw std::runtime_error("MFD3D12VideoCapture::open: requested subtype is not supported by CPU native upload path");
        }
        ...
        lastError_ = internal::MakeError(L"MFD3D12VideoCapture::open", internal::Utf8ToWide(e.what()));
        close();
        return false;
    }
}

MFD3D12VideoReadResult MFD3D12VideoCapture::read() {
    MFD3D12VideoReadResult result;
    if (!impl_ || !impl_->opened) {
        result.status = MFFrameStatus::NotOpened;
        result.error = internal::MakeError(L"MFD3D12VideoCapture::read", L"capture is not opened");
    ...
    if (cpu.status == MFFrameStatus::EndOfStream && impl_->config.loop) {
        if (!impl_->reader.seek(impl_->config.startPosition100ns)) {
            ...
            result.error = internal::MakeError(L"MFD3D12VideoCapture::read", L"loop seek failed");
            lastError_ = result.error;
            return result;
        }
    ...
    try {
        ...
        result.error = internal::MakeError(L"MFD3D12VideoCapture::read/D3D12Processing", internal::Utf8ToWide(e.what()));
        lastError_ = result.error;
        return result;
    } catch (const std::exception& e) {
        result.status = MFFrameStatus::Error;
        result.error = internal::MakeError(L"MFD3D12VideoCapture::read", internal::Utf8ToWide(e.what()));
        lastError_ = result.error;
        return result;
    }
}

bool MFD3D12VideoCapture::seek(std::int64_t position100ns) {
    if (!impl_ || !impl_->opened) {
        lastError_ = internal::MakeError(L"MFD3D12VideoCapture::seek", L"capture is not opened");
        return false;
    }
    const bool ok = impl_->reader.seek(position100ns);
    if (!ok) {
        lastError_ = internal::MakeError(L"MFD3D12VideoCapture::seek", L"IMFSourceReader::SetCurrentPosition failed");
    }
    return ok;
}

void MFD3D12VideoCapture::close() noexcept {
    if (!impl_) return;
    impl_->opened = false;
    impl_->framePool.close();
    impl_->reader.close();
...
bool MFD3D12VideoCapture::isOpened() const noexcept {
    return impl_ && impl_->opened;
}

std::int64_t MFD3D12VideoCapture::duration100ns() const noexcept {
    return impl_ ? impl_->reader.duration100ns() : -1;
}

const std::wstring& MFD3D12VideoCapture::filePath() const noexcept {
