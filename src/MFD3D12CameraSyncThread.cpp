...
struct MFD3D12CameraSyncThread::Impl {
    MFD3D12CameraFrameQueuePtr leftQueue;
    MFD3D12CameraFrameQueuePtr rightQueue;
    MFD3D12StereoFrameQueuePtr output;
    MFD3D12CameraSyncThreadConfig config;
    ...
    CandidatePair findBestPair(const std::deque<MFD3D12CameraFrame>& left,
                               const std::deque<MFD3D12CameraFrame>& right) const {
        CandidatePair best;
    ...
    void trimCapacity(std::deque<MFD3D12CameraFrame>& left,
                      std::deque<MFD3D12CameraFrame>& right) {
    ...
    void trimImpossible(std::deque<MFD3D12CameraFrame>& left,
                        std::deque<MFD3D12CameraFrame>& right) {
    ...
    void drainInputs(std::deque<MFD3D12CameraFrame>& left,
                     std::deque<MFD3D12CameraFrame>& right) {
    ...
    void emitPairs(std::deque<MFD3D12CameraFrame>& left,
                   std::deque<MFD3D12CameraFrame>& right) {
        for (;;) {
    ...
    void workerLoop(const ThreadKit::StopToken& token) {
        std::deque<MFD3D12CameraFrame> left;
        std::deque<MFD3D12CameraFrame> right;
        ...
        while (!token.stopRequested()) {
            ...
            if (left.empty() || right.empty()) {
                // Wait briefly on the side that is currently empty. We still use
                // tryPopAll() above so bursts are drained efficiently.
                if (left.empty()) {
...
MFD3D12CameraSyncThread::MFD3D12CameraSyncThread()
...
bool MFD3D12CameraSyncThread::open(MFD3D12CameraFrameQueuePtr leftQueue,
                                   MFD3D12CameraFrameQueuePtr rightQueue,
    ...
    impl_->config = config;
    impl_->output = std::make_shared<MFD3D12StereoFrameQueue>(
...
void MFD3D12CameraSyncThread::start() {
    if (!impl_ || !impl_->opened) throw std::logic_error("MFD3D12CameraSyncThread::start: not opened");
    if (impl_->worker.joinable()) throw std::logic_error("MFD3D12CameraSyncThread::start: already started");
    impl_->worker.start([this](const ThreadKit::StopToken& token) { impl_->workerLoop(token); });
}

void MFD3D12CameraSyncThread::requestStop() noexcept { if (impl_) impl_->worker.requestStop(); }
void MFD3D12CameraSyncThread::stop() { if (impl_) impl_->worker.stopAndJoin(); }

void MFD3D12CameraSyncThread::close() noexcept {
    if (!impl_) return;
...
bool MFD3D12CameraSyncThread::isOpened() const noexcept { return impl_ && impl_->opened; }
bool MFD3D12CameraSyncThread::isRunning() const noexcept { return impl_ && impl_->worker.isRunning(); }
MFD3D12StereoFrameQueuePtr MFD3D12CameraSyncThread::outputQueue() const noexcept { return impl_ ? impl_->output : nullptr; }

MFD3D12CameraSyncThreadStats MFD3D12CameraSyncThread::stats() const noexcept {
    MFD3D12CameraSyncThreadStats s;
    if (!impl_) return s;
...
void MFD3D12CameraSyncThread::resetStats() noexcept {
    if (!impl_) return;
...
std::int64_t MFD3D12CameraSyncThread::currentBaselineDiff100ns() const noexcept {
    return impl_ ? impl_->currentBaseline() : 0;
}

std::exception_ptr MFD3D12CameraSyncThread::workerException() const {
    return impl_ ? impl_->worker.exception() : nullptr;
}

void MFD3D12CameraSyncThread::rethrowWorkerExceptionIfAny() const {
