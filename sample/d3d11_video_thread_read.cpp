...
void PrintUsage() {
    std::wcout << L"usage:\n"
               << L"  d3d11_video_thread_read <file> <width> <height> <fpsNum> <fpsDen> <subtype> <shaderDir> [frameCount] [fast]\n"
    ...
    try {
        ...
        while (received < frameCount) {
            ...
            std::wcout << L"frame " << received
                       ...
                       << L" size=" << item->width() << L"x" << item->height()
                       << L" format=" << DxgiFormatName(item->format())
                       << L" resource=" << (item->resource().Get() ? L"yes" : L"no")
                       << L" srv=" << (item->srv() ? L"yes" : L"no") << L"\n";
            ++received;
        }
        ...
        std::wcout << L"stats read=" << s.framesRead
                   ...
                   << L" readFailures=" << s.readFailures
                   << L" cloneFailures=" << s.cloneFailures << L"\n";
