...
- `v1.1.0` の主な更新は、新 D3DHelper 対応、D3D11 backend 追加、D3D11 smoke test / sample 追加です。
...
## v1.1.0 verification status

以下は `feature/d3d11-backend-v1.1.0` で確認済みです。

- CMake configure / build
- 全体 CTest
- D3D11 camera single-read smoke
- D3D11 video single-read smoke
- D3D11 camera thread smoke
- D3D11 video thread smoke
- D3D11 sync thread smoke は実装・CTest 登録済みです。2台カメラが無い環境では skip されます。
- video decode の visible size / coded size 差異への対応。例: `2880x1404` で open し、read 後に decoder が `2880x1408` padded sample を返すケース。
...
D3D11 の capture thread / sync thread / sample では、同一 `D3D11Core` を複数 thread から使うため、アプリケーション側の `D3D11CoreConfig` で `enableMultithreadProtection = true` を有効にすることを推奨します。
...
- Media Foundation decode to requested CPU output format
- EOF handling
- loop playback
- seek by 100 ns Media Foundation time
- real-time playback by timestamp
- decode-as-fast-as-possible playback
- small decoder padding tolerance after first read. This handles cases where the visible size requested at `SetCurrentMediaType()` time differs from the coded sample size reported by the decoder.
...
`MFFRAMESOURCE_INSTALL` は既定で `OFF` です。install / CMake package export が必要な場合のみ `ON` にしてください。
...
Video input は Media Foundation decoder の出力形式に対して request を出します。
...
一部の動画では、プロパティ上の visible size と decoder 内部の coded size が異なることがあります。たとえば `2880x1404` で open した後、decoder が `2880x1408` の padded sample を返す場合があります。MFFrameSource は小さな padding 増加を受け入れ、実 sample layout に合わせて内部 upload pool を更新します。

## Samples

### D3D11 samples

D3D11 samples are CLI-oriented validation samples. They print frame metadata and confirm resource / SRV availability.

Camera list / single read:

```bat
out\build\default\Debug\d3d11_camera_read.exe list

out\build\default\Debug\d3d11_camera_read.exe read 0 1920 1080 60 1 NV12 C:\path\to\D3D11Helper\shaders\D3D11Processing
```

Camera thread:

```bat
out\build\default\Debug\d3d11_camera_thread_read.exe 0 1920 1080 60 1 NV12 C:\path\to\D3D11Helper\shaders\D3D11Processing 300
```

Stereo sync:

```bat
out\build\default\Debug\d3d11_stereo_sync.exe 0 1 1920 1080 60 1 NV12 C:\path\to\D3D11Helper\shaders\D3D11Processing 300
```

Video read:

```bat
out\build\default\Debug\d3d11_video_read.exe C:\path\to\input.mp4 1920 1080 60 1 NV12 C:\path\to\D3D11Helper\shaders\D3D11Processing 300
```

Video thread:

```bat
out\build\default\Debug\d3d11_video_thread_read.exe C:\path\to\input.mp4 1920 1080 60 1 NV12 C:\path\to\D3D11Helper\shaders\D3D11Processing 300 1
```

### D3D12 samples

D3D12 samples include preview / recording samples for some paths.
...
```bat
out\build\default\Debug\d3d12_video_thread_read.exe C:\path\to\input.mp4 1920 1080 60 1 NV12 C:\path\to\D3D12Helper\shaders\D3D12Processing 300 1
```
...
D3D11 stereo sync smoke:

```bat
set "MFFRAMESOURCE_TEST_LEFT_CAMERA_INDEX=0"
set "MFFRAMESOURCE_TEST_RIGHT_CAMERA_INDEX=1"
set "MFFRAMESOURCE_TEST_CAMERA_WIDTH=1920"
set "MFFRAMESOURCE_TEST_CAMERA_HEIGHT=1080"
set "MFFRAMESOURCE_TEST_CAMERA_FPS_NUM=60"
set "MFFRAMESOURCE_TEST_CAMERA_FPS_DEN=1"
...
ctest --test-dir out/build/default -C Debug -R test_d3d11_sync_thread_smoke --output-on-failure
```
...
- D3D11 sample の preview / video recording path
